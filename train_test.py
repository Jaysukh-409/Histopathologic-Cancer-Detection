...
def train_model(
    EPOCHS, clip_norm, net, DEVICE, train_dataloader, val_dataloader, max_ler_rate
):
    # EPOCHS = 10
    # clip_norm = True
    lr_schedule = lambda t: np.interp(
        ...
        [0, max_ler_rate, max_ler_rate / 20.0, 0],
    )[0]
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            f"Train Acc: {train_acc/n:.4f}, Training Time for 1 Epoch: {time.time() - start:.1f}, lr: {lr:.6f}"
        )
        ...
        with torch.no_grad():
            ...
            print(
                f"Epoch: {epoch+1} | Validation Loss: {val_loss/m:.4f}, Validation Acc: {val_acc/m:.4f}, Inference Time: {time.time() - start:.1f}, lr: {lr:.6f}"
            )
